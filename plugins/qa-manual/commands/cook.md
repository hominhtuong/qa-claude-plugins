---
description: Generate TEST CASES from a requirement/spec/Figma or test plan → xlsx + (preferably) Google/Lark Sheet; parallel multi-phase when large
argument-hint: <path/to/plan.md | feature name + spec/Figma link>
allowed-tools: Read, Glob, Grep, Edit, Write, Bash, Agent
---

# /cook — Generate test cases
...
3. Is there already a test plan from `/qa-manual:plan-tests`? If so => read that plan as the main input.
...
- **Path to a plan `.md`** (from `/qa-manual:plan-tests`) → MODE A (multi-phase).
...
- If there's no plan and the work is large (>50 TCs, many modules) → suggest `/qa-manual:plan-tests` first.
