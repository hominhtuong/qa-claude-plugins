---
description: One-click setup of the qa plugin in THIS project — you (Claude) auto-detect the OS and run scripts/setup.py yourself; it creates .claude/qa-claude/ (config + .env), patches .gitignore, and checks the toolchain. The user does NOT run any terminal command.
...
The script installs into `.claude/qa-claude/`: `.env` + `log-bug.config.yml` (yours, never overwritten) and `.env.example` / `log-bug.config.example.yml` / `testcase-template.md` (refreshed), patches `.gitignore`, and runs the toolchain **doctor**.
...
- To turn on notifications / report sharing: open **`.claude/qa-claude/.env`**, flip the `ENABLE_*` flag you want and fill that section (Lark / Slack / R2 / S3). All optional — skip to use nothing.
- To use `/qa:log-bug`: open **`.claude/qa-claude/log-bug.config.yml`** and fill in your Lark board id + dev mapping (or run `/qa:update-board <board-url>`).
- Both files are yours and survive future `setup` runs.

## Principles
- The user never touches a terminal — you run the script and summarize.
- Re-running is safe (idempotent): your `.env`/`log-bug.config.yml` are never overwritten; only the `.example`/template references refresh.
