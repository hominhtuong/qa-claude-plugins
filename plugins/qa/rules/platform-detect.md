...
A static rule shared by every automation-mode command. This is a multi-platform automation project (Web Playwright + App Appium iOS/Android) → each command must **lock onto exactly one platform** then only read that platform's skills. Goal: **don't read extra skills** (save tokens), don't run the wrong runtime.
