...
> - **Screenshot evidence**: saved in `results/<feature-name>/screenshots/`, **named by BUG-ID** (`02-APP01-published-tab-sqlite-crash.png`). Each `[APP-BUG]` MUST have at least one screenshot.
> - Each bug is also **appended to the cross-feature defect register** `results/bug-summary.md` (assign an APP-ID, update the count table + severity distribution).
...
- **Bằng chứng**: `results/<feature-name>/screenshots/<file>.png`.
