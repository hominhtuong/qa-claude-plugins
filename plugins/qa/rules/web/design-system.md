...
Each screen = one file `sitemap/screens/<id>.json` (each member owns their own ⇒ fewer conflicts). `/qa:exploratory` (every screen visited) / `/qa:plan-tests` (list only) / `/qa:cook` create/update a node (fields: `realName`, `testFeature`, `platform`, `screenClass`, `route` (URL), `parentId`, `keyElement`, `reach` (route+clicks), `notes`; `childrenIds` auto-derived). Schema: `sitemap/SCHEMA.md`. Rebuild the merged `sitemap/sitemap.json` with `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/gen_sitemap.py` (or the project's own listener if it has one) — do NOT edit `sitemap.json` by hand.
...
- **Each run = one directory** `results/tests/<ddMMMyyyy>/<runTs>/`: ExtentReports Spark HTML + `screenshots/` (pass & fail) + `traces/` + `videos/`. Each test embeds a screenshot of both states + **the URL at the end**. Do NOT write artifacts to top-level `results/`. End of run: push R2/S3 + notify (enabled in `.claude/qa-claude/.plugin.env`).
