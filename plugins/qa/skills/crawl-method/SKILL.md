...
3. **For each screen popped from the frontier:**
   a. **Confirm you're on it** — capture the screen (web: `browser_snapshot`; app: `appium_get_page_source`, or `appium_screenshot` with **`maxWidth: 720`** per [capture-screenshot](../capture-screenshot/SKILL.md) so a long crawl doesn't exhaust the many-image cap) and pick the `keyElement` (the stable anchor that proves the screen is shown).
