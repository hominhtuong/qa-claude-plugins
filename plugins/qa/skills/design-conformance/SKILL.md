...
3. **Screenshot the app**: `mcp__plugin_qa_appium__appium_take_screenshot` (bundled Appium MCP; if the project uses its own Appium MCP the tool may be `mcp__appium__appium_take_screenshot` — locate via ToolSearch) → JPG, **pass the full relative path as the `filename`**: `sitemap/screenshots/<name>.jpg` (a bare file name falls back to the MCP output dir / project root; NOT to Desktop, NOT PNG).
