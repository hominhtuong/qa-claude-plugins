...
> This skill ONLY extracts + picks iOS locators; writing them into Java fields is the `declare-screen` skill. `iosNsPredicate`/`iosClassChain` are **iOS-only** (Android skips them). Cross-platform elements with differing locators between the two OSes → combine with `@MobileFindBys({ @MobileFindBy(uiautomator=...), @MobileFindBy(iosClassChain=...) })` (Android entry first, iOS second). [MCP Screenshot Rule] screenshots via MCP use JPG, saved to `sitemap/screenshots/<name>.jpg`, NOT to Desktop.
