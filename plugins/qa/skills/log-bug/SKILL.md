...
1. **Load config** from `.claude/qa-claude/log-bug.config.yml` (generated by skill `setup`, edited by the user):
   ...
   - **Missing config** → tell the user to run `/qa:setup-plugin`; or if the user pasted a board URL, extract `base_id`/`table_id` from `.../base/<base_id>?table=<table_id>` and proceed.
