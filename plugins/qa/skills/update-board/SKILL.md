...
## Procedure
1. **Locate config**: `.claude/qa-claude/log-bug.config.yml`. Missing → tell the user to run `/qa:setup` first.
...
## Rules
- Edit ONLY `.claude/qa-claude/log-bug.config.yml`. Never touch `.claude/qa-claude/.plugin.env` secrets, never print tokens.
