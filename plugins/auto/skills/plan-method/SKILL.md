...
Reusable capability: from a feature **that passed a clean exploratory pass** → a **firm plan (.md)** describing how to build the tests, for `/cook` to execute. **Do NOT write code** at this step. Platform-agnostic: the command passes `<platform>` (web/app) → read the right rules `../../rules/<platform>/design-pattern.md`.
...
2. **Survey for reuse**: Glob the feature's existing Screen/Test → reuse `base`/`utils`/`actions`/`models`, no duplication. Element source: Screen.java → elements.json → (if missing) note "needs MCP/DOM discovery" for `/cook` to run.
...
4. **Layering principle** (write into the plan): `screens` (no-assert) → `tests` (assert) → `regression/smoke` (compose). Each Screen described via the `declare-screen` skill (key element, action = verb). Element expected to lack an id → note it for `/cook` to RECORD via the `missing-ids` skill.
5. **Correct expectations**: the plan records the **canonical expectation** (per spec/original app) for each test so `/cook` keeps the test honest. The sitemap only **lists** screens to create/update — `/cook` is what actually writes them.
6. **Conclude**: print the plan path + a 3-5 line summary for the user to review before `/cook`.
